...
개발 노트:
  ...
  multer 라이브러리를 사용해서 POST로 온 파일 업로드 요청을 처리하도록 router을 설정하였음

  이제 ckeditor에서 사용자가 이미지를 선택하면 /uploads/images로 POST 요청을 하는 것을 구현해야하는데

  무슨 문제가 있는지 ckeditor가 CustomUploadAdapter를 인식하지 못하고 있음

  다음 작업 때 문제점을 파악해서 정상적으로 이미지 업로드가 되도록 해야하겠음

  // 20.09.26

  ckeditor.js를 클라이언트 상에서 실행되게 하고 사용자가 에디터에서 이미지를 첨부하면 POST로 /uploads/images에 비동기 통신, json을 요청함

  uploads 라우터에서 해당 파일을 받아서 서버에 저장하고 파일의 이름, 용량 등의 메타 데이터는 db에 저장함

  그리고 나서 ckeditor에게 이미지 파일의 url을 json형태로 반환해줌

  url을 성공적으로 받으면 ckeditor은 img 태그로 해당 이미지를 에디터상에 띄워줌

  이제 글 작성, 수정 시에 이미지를 첨부할 수 있게 됐지만 현재 files 테이블에는 단순한 이미지 정보만 있고 각 이미지가 속한 post_id 값이 없는 상태임

  글 작성 시에 post_id가 주어지기 때문에 첨부하는 즉시 post_id를 할당해줄 수 없기 때문에

  글 작성 버튼을 누르면 content의 내용을 파싱해서 각 이미지 url을 뽑고 그 url을 조건값으로 files 테이블에서 조회, post_id 할당하는 작업을 거쳐야 함

  다음 작업에는 post_id 할당하는 것을 하고, 글 수정이나 삭제 시에 글내용에서 누락된 사진은 서버에서 자동으로 지워지게 구현할 예정이다.

  // 20.09.27

  글을 작성하면 글의 내용을 파싱해서 img태그 안에 있는 이미지 url들을 가져옴
  
  그리고 File DAO에 assign, unassign을 추가했음
  
  만약에 글을 작성하면 이미지 태그들의 url을 가져오는데, 가져온 url을 기준으로 assign DAO가 files 테이블 각 파일 레코드의 post_id를 해당 글의 id로 지정함 

  그렇게 되면 각 post마다 어떤 이미지 파일을 가지고 있는지 식별이 가능해짐

  만약의 글 내용을 수정하면 unassign이 그 post_id에 해당하는 파일 레코드 post_id를 NULL로 초기화시키고 다시 assign을 이용해 현재 이미지 url들로 재할당함

  글 삭제의 경우는 unassign만 하면 되니 간단함

  위의 모든 과정을 거치면 실제 게시물에서 사용되는 사진들은 post_id가 NULL이 아니므로

  서버에서는 주기적으로 post_id가 NULL인 데이터를 지우는 것으로 가비지 컬렉션이 가능해짐

  추가로 post를 find하는 쿼리에 thumnail 사진 url도 가져오도록 수정했음 (join)

  thumnail 사진 기준은 해당 글에서 첫번째로 업로드된 사진임

  다음 작업엔 user 페이지를 구현해보자

  // 20.09.29

  대학교 수업에 알고리즘 공부까지 겸하다 보니 작업할 시간이 많이 나지않아 아쉬움

  간단하게 자신의 개인 정보를 확인하고 수정할 수 있는 form을 만들었음

  이제 modify 버튼을 누르면 put 메서드로 받아서 처리할 수 있게끔 구현해야 하고

  추가로 현재 사용자가 자신이 맞는지 edit 페이지 가기 전에 다시 로그인을 하도록 해야겠음

  // 20.09.30

  유저 정보 수정 페이지에서 회원가입때와 마찬가지로 입력값 검증 기능을 추가했음

  차이가 있다면 패스워드를 바꿀 것이 아니면 패스워드 란은 비워도 상관없기 때문에 암호화하는 모듈에 패스워드가 비어있을 경우 등 조건을 추가함

  그리고 다른 사람이 url을 통해 정보를 열람하고 수정하는 것을 방지하기 위해서 로그인 상태, 현재 로그인한 user_id를 비교해서

  다른 사용자가 열람하는 시도가 있으면 홈화면으로 팅겨나게 구현했음

  이제 유저마다 자신만의 프로필 사진을 설정하고 글을 쓰거나 하면 보여지게 만들면 될거 같다.

  // 20.10.02

  유저 정보 수정 페이지에서 프로필 사진을 추가할 수 있게끔 하였고

  users 테이블에 추가된 프로필 사진의 파일 경로를 담는 profile 컬럼을 추가함

  이제 게시물을 열람할 때 댓글란에 각 사용자별로 지정된 프로필 사진이 보여지게 됨

  기본 프로필 이미지도 만들어놔서 프로필 사진이 따로 없는 유저는 기본 프로필 이미지를 사용하게 됨

  이제 기본적으로 구현할 건 거의 다 한거 같다.

  추가로 더 해볼만한건 socket.io를 이용해서 채팅방 만들기랑 JWT방식을 사용한 로그인 REST API 만들기다.

  이 두개는 짬짬히 시간이 날 때마다 해봐야겠음

  // 20.10.09

  간단하게 비로그인 상태에서 채팅을 할 수 있는 페이지를 만들었음

  socket.io를 설치해서 세팅을 해놓고 사용자가 chat 페이지로 오면

  socket.io의 io() 생성자로 socket을 서버로부터 하나 얻어서 자신 또는 다른 누군가가 채팅을 치면 서버에서 채팅 메시지를 관리, 각 소켓에게 전달해줘
  
  다른 socket을 가지고 있는 사용자와 교류할 수 있게 되는 것이다.

  현재는 비로그인 상태일 때 닉네임이 guest숫자인데 로그인 상태면 각 계정에 등록된 nickname으로 소통할 수 있도록 구현해야 할 것임

  그리고 채팅 참여자 목록을 채팅로그 박스 옆에 만들어서 현재 몇 명이, 누가 채팅을 하고 있는지 알 수 있게끔 만들 것이다.

  // 20.10.10

  비로그인 상태면 not loggined 이벤트를 발생시켜 guest숫자로 nickname를 저장하고 사용자 정보를 서버 userList에 추가함

  로그인 상태면 현재 사용자의 닉네임을 nickname으로 가정하고 사용자 정보를 서버 userList에 추가함

  닉네임 설정이 되면 서버는 receive userList란 이벤트를 userList의 value값들과 함께 모든 소켓에게 전달하는데,

  userList를 전달받은 각 소켓은 채팅로그 우측에 있는 유저리스트칸에 userList의 요소들을 집어넣음

  그렇게되면 현재 소켓이 할당된, 즉 채팅을 이용하고 있는 사용자들의 닉네임이 보여지게 됨

  그리고 userList의 length로 현재 이용자 수를 알아내서 채팅 페이지 타이틀에 명시했음

  문제는 새로고침할 때마다 채팅 로그가 삭제되고 새로운 소켓이 재할당되기 때문에 같은 계정이라도
  
  창을 2개 띄워놓으면 채팅 이용자에 같은 닉네임을 가진 이용자가 2명 있게 된다.

  따라서 소켓을 사용하되, 채팅 이용자 관리는 세션 정보를 중심으로 하는게 낫겠다는 생각이 들었음

  그리고 채팅 내역은 key-value 구조인 NOSQL, Redis를 이용해서 메모리에 저장했다가 채팅 페이지에 들어오면 읽어오도록 구현해야겠다.

  // 20.10.11

  우선 동일 이용자가 다른 탭이던가 브라우저에서 접속했을 때, 즉 기존 이용자와 같은 닉네임을 가진 사람이 접속했을 때 new connection 이벤트를 발생시켜

  기존의 이용자는 채팅을 이용할 수 없게 disconnection 해버리고 새로운 이용자만이 채팅을 이용할 수 있게 구현했음

  세션 정보를 이용하려면 session.socket.io 라이브러리를 쓰면 되는데 딱히 세션을 공유할만큼의 로직은 아닌거 같아서 닉네임이 중복인지 확인하는 방법으로 해결했다.

  채팅 메시지 저장은 redis를 쓸 것이라고 했는데, 검색해보니 redis는 여러 서버(포트)에서 채팅 서비스를 공유하여 pub/sub하기에 더 알맞다고 하였다.

  그래서 다들 mysql과 같이 db에 저장하는 방법을 주로 사용하길래 다음에 작업할 때 mysql에 테이블을 하나 만들어서 채팅 기록을 저장해야겠다.

  // 20.10.16

  chat 테이블을 하나 만들어서 사용자가 채팅을 전송할 때 chat 테이블에 발신인 nickname과 message를 저장하고,

  새로운 사용자가 채팅 서비스에 접속하면 chat 테이블에 저장되어 있는 채팅 쿼리들을 읽어와 채팅방에 보여주게 구현했음

  모든 쿼리를 보여주면 시간에 따라 부하가 커지므로 limit 100으로 최근 채팅 100개만 보여주도록 하였음

  중간고사 기간이라 시간이 많이 부족한데, 짬내서 jwt까지 달려봐야겠다. 힘내자

  // 20.11.1

  jwt는 토큰 인증 방식에서 쓰이는 토큰의 한 종류인데, 세션을 사용하지 않기 때문에 RESTFUL한 시스템에서 주로 사용한다고 함

  그래서 react를 공부할 겸, rest api서버와 프론트 서버가 따로 있는 구조로 새로 프로젝트를 만들고 jwt를 적용해봐야겠다.